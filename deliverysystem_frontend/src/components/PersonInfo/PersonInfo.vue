<template>
  <div class="container">
    ...
    <form class="personal-info-form">
      ...
      <input type="text" name="name" id="name" v-model="username" required @blur="validateName">
      <div class="error-message" v-if="nameError">{{ nameError }}</div>
      <label for="gender">性别：</label>
      <input type="radio" name="gender" id="male" value="male" v-model="gender" required>男&nbsp;&nbsp;&nbsp;
      <input type="radio" name="gender" id="female" value="female" v-model="gender">女
      <label for="birthday">生日：</label>
      <input type="date" name="birthday" id="birthday" v-model="birthday" required @blur="validateBirthday">
      <div class="error-message" v-if="birthdayError">{{ birthdayError }}</div>
      <label for="email">邮箱：</label>
      <input type="email" name="email" id="email" v-model="email" required @blur="validateEmail">
      <div class="error-message" v-if="emailError">{{ emailError }}</div>
      <label for="password">密码：</label>
      <input type="password" name="password" id="password" minlength="6" v-model="password" required @blur="validatePassword">
...
export default {
  ...
  methods: {
    ...
    validatePhoneNumber() {
      ...
      } else if (!/^1[0-9]{10}$/.test(this.phoneNumber)) {
